"""
This is a slightly updated version of gathering.py (https://github.com/nccgroup/PMapper/blob/master/principalmapper/graphing/gathering.py).
update_admin_status() is the only function got updated. 
The script will replace the original gathering.py in PMapper
...
def update_admin_status(nodes: List[Node], scps: Optional[List[List[dict]]] = None) -> None:
    ...
    for node in nodes:
        node.is_admin = is_admin(node, scps)

def is_admin(node: Node, scps: Optional[List[List[dict]]] = None) -> Boolean:
    ''' An admin needs to pass all the tests '''
    ...
    if not query_interface.local_check_authorization_handling_mfa(node, action, node.arn, {},
    ...
    if not query_interface.local_check_authorization_handling_mfa(node, action, node.arn, condition_keys,
                                                                service_control_policy_groups=scps)[0]:
        return False

    # check if node can create a role and attach the AdministratorAccess policy or an inline policy
    if not query_interface.local_check_authorization_handling_mfa(node, 'iam:CreateRole', '*', {})[0]:
        return False
    else:
        if not query_interface.local_check_authorization_handling_mfa(node, 'iam:AttachRolePolicy', '*',
                                                                    condition_keys,
                                                                    service_control_policy_groups=scps)[0]:
            return False
        if not query_interface.local_check_authorization_handling_mfa(node, 'iam:PutRolePolicy', '*', condition_keys,
                                                                    service_control_policy_groups=scps)[0]:
            return False
    ...
    for attached_policy in node.attached_policies:
        if attached_policy.arn != node.arn and ':aws:policy/' not in attached_policy.arn:
            if not query_interface.local_check_authorization_handling_mfa(node, 'iam:CreatePolicyVersion',
                                                                        attached_policy.arn, {},
                                                                        service_control_policy_groups=scps)[0]:
                return False
    ...
    if node_type == 'user':
        for group in node.group_memberships:
            if not query_interface.local_check_authorization_handling_mfa(node, 'iam:PutGroupPolicy', group.arn, {},
                                                                        service_control_policy_groups=scps)[0]:
                return False
            if not query_interface.local_check_authorization_handling_mfa(node, 'iam:AttachGroupPolicy', group.arn,
                                                                        condition_keys,
                                                                        service_control_policy_groups=scps)[0]:
                return False
    ...
    return _rnd_action_check(node)

def _rnd_action_check(node: Node) -> Boolean:
    ''' Check if node can access a set of randomly chosen actions. 
    Return the True if the node passes all the test '''
    test_cnt = 10
    pass_cnt = 0
    with open('./misc/aws_actions.txt') as fp:
        content = fp.readlines()
        length = len(content)
        for _ in range(0,test_cnt):
            action = content[random.randint(0, length-1)]            
